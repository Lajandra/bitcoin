...
>>>>>>> 0a93a166146 (interfaces, refactor: Add more block information to block connected notifications)
||||||| parent of 4bac6e9e6c6 (indexes, refactor: Remove UndoReadFromDisk calls from indexing code)
        m_notifications->blockConnected(node::MakeBlockInfo(index, block.get()));
=======
...
>>>>>>> 4bac6e9e6c6 (indexes, refactor: Remove UndoReadFromDisk calls from indexing code)
    }
...
public:
    ...
    std::unique_ptr<Handler> attachChain(std::shared_ptr<Notifications> notifications, const CBlockLocator& locator, const NotifyOptions& options) override
    {
        ...
        block_info.emplace(MakeBlockInfo(start_block));
        ...
        handler->m_thread_sync = std::thread(&util::TraceThread, options.thread_name,
            ...
            promise.set_value();
            SyncChain(active.m_chain, start_block, notifications, interrupt,
