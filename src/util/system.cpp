...
||||||| parent of aa3dec078fc (refactor: Implement missing error checking for ArgsManager flags)
 * Check settings value validity according to flags.
=======
 * Interpret settings value and apply ALLOW_* type checking flags.
>>>>>>> aa3dec078fc (refactor: Implement missing error checking for ArgsManager flags)
 *
...
=======
    ...
    if (negated) {
        ...
        if (!(flags & (ArgsManager::ALLOW_BOOL | ArgsManager::ALLOW_STRING))) {
            error = strprintf("Negating of -%s is meaningless and therefore forbidden.", key);
            return std::nullopt;
        }
        return util::SettingsValue{false};
