...
protected:
    ...
    {
        ConstructWithFn([this](auto&& ...x) { this->InitValue(std::forward<decltype(x)>(x)...); }, std::forward<Args>(args)...);
    }

    template<typename Fn, typename...Args>
    void ConstructWithFn(const Fn& fn, Args&&... args)
    {
        fn(std::forward<Args>(args)...);
    }

    template<typename Fn, typename...Args>
    void ConstructWithFn(const Fn& fn, Error error, Args&&... args)
    {
        this->AddError(std::move(error.message));
        ConstructWithFn([this](auto&& ...x) { this->InitFailure(std::forward<decltype(x)>(x)...); }, std::forward<Args>(args)...);
    }

    template<typename Fn, typename...Args>
    void ConstructWithFn(const Fn& fn, Warning warning, Args&&... args)
    {
        this->AddWarning(std::move(warning.message));
        ConstructWithFn(fn, std::forward<Args>(args)...);
    }

    template<typename Fn, typename OT, typename OF, typename...Args>
    void ConstructWithFn(const Fn& fn, Result<OT, OF>&& other,  Args&&... args)
    {
        *this << std::move(other);
        ConstructWithFn(fn, std::forward<Args>(args)...);
    }
...
public:
    ...
    //! Move warning/error messages from one result to another.
