...
<<<<<<< HEAD
                ...
                if (chainman.m_blockman.m_have_pruned && check_blocks > MIN_BLOCKS_TO_KEEP) {
                    LogPrintfCategory(BCLog::PRUNE, "pruned datadir may not have more than %d blocks; only checking available blocks\n",
                                      MIN_BLOCKS_TO_KEEP);
||||||| parent of fbcc7769749 (refactor: Reduce number of LoadChainstate parameters)
                auto check_blocks = args.GetIntArg("-checkblocks", DEFAULT_CHECKBLOCKS);
                if (chainman.m_blockman.m_have_pruned && check_blocks > MIN_BLOCKS_TO_KEEP) {
...
=======
                if (chainman.m_blockman.m_have_pruned && options.check_blocks > MIN_BLOCKS_TO_KEEP) {
                    LogPrintf("Prune: pruned datadir may not have more than %d blocks; only checking available blocks\n",
                              MIN_BLOCKS_TO_KEEP);
>>>>>>> fbcc7769749 (refactor: Reduce number of LoadChainstate parameters)
                }
                maybe_verify_error = VerifyLoadedChainstate(chainman, options);
||||||| parent of 65d4469e323 (refactor: Reduce number of LoadChainstate return values)
                uiInterface.InitMessage(_("Verifying blocks…").translated);
                if (chainman.m_blockman.m_have_pruned && options.check_blocks > MIN_BLOCKS_TO_KEEP) {
                    LogPrintf("Prune: pruned datadir may not have more than %d blocks; only checking available blocks\n",
                              MIN_BLOCKS_TO_KEEP);
...
=======
                return f();
>>>>>>> 65d4469e323 (refactor: Reduce number of LoadChainstate return values)
            } catch (const std::exception& e) {
                LogPrintf("%s\n", e.what());
                return std::make_tuple(node::InitStatus::FAILURE, _("Error opening block database"));
            }
        };
        auto [status, error] = catch_exceptions([&]{ return LoadChainstate(chainman, cache_sizes, options); });
        if (status == node::InitStatus::SUCCESS) {
            uiInterface.InitMessage(_("Verifying blocks…").translated);
            if (chainman.m_blockman.m_have_pruned && options.check_blocks > MIN_BLOCKS_TO_KEEP) {
                LogPrintf("Prune: pruned datadir may not have more than %d blocks; only checking available blocks\n",
                          MIN_BLOCKS_TO_KEEP);
        ...
        if (status == node::InitStatus::FAILURE_INCOMPATIBLE_DB) {
            return InitError(error);
        }
