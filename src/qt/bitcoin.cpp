...
||||||| parent of 5da717e04ea (scripted-diff: Remove double newlines after some init errors)
    if (!CheckDataDirOption()) {
        InitError(strprintf(Untranslated("Specified data directory \"%s\" does not exist.\n"), gArgs.GetArg("-datadir", "")));
=======
    if (!CheckDataDirOption()) {
        InitError(strprintf(Untranslated("Specified data directory \"%s\" does not exist."), gArgs.GetArg("-datadir", "")));
>>>>>>> 5da717e04ea (scripted-diff: Remove double newlines after some init errors)
        QMessageBox::critical(nullptr, PACKAGE_NAME,
    ...
    try {
        ...
        if (!gArgs.ReadConfigFiles(error, true)) {
            ...
            return EXIT_FAILURE;
||||||| parent of 15cb0a2492d (Deduplicate bitcoind and bitcoin-qt init code)
    /// 6a. Determine availability of data directory
    if (!CheckDataDirOption()) {
        InitError(strprintf(Untranslated("Specified data directory \"%s\" does not exist."), gArgs.GetArg("-datadir", "")));
        QMessageBox::critical(nullptr, PACKAGE_NAME,
...
>>>>>>> 15cb0a2492d (Deduplicate bitcoind and bitcoin-qt init code)
        }
        return EXIT_FAILURE;
    }
#ifdef ENABLE_WALLET
