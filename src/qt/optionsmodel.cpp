...
//! it must be negated or unset.
//! @throws std::runtime_error if value has non-string type like integer, bool,
//! array, or object.
...
QVariant OptionsModel::getOption(OptionID option) const
{
    ...
    // default proxy
    case ProxyUse:
        return ParseProxyString(ToQString(node().getPersistentSetting("proxy"))).is_set;
    case ProxyIP:
        return m_proxy_ip;
    case ProxyPort:
        return m_proxy_port;
    ...
    // separate Tor proxy
    case ProxyUseTor:
        return ParseProxyString(ToQString(node().getPersistentSetting("onion"))).is_set;
    case ProxyIPTor:
        return m_onion_ip;
    case ProxyPortTor:
        return m_onion_port;
