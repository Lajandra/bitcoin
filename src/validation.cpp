...

        {
            ...
            if (pindexFork != pindexNewTip) {
                ...
                if (kernel::IsInterrupted(m_chainman.GetNotifications().blockTip(GetSynchronizationState(fInitialDownload), *pindexNewTip))) {
                    // Just breaking and returning success here. This could be
                    // changed to bubble up the kernel::Interrupted value to the
                    // caller so the caller could distinguish between completed
