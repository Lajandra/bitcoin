...
<<<<<<< HEAD
        if (!LookUpOne(*m_db, pindex, entry)) {
            return error("%s: Cannot read current %s state; index may be corrupted",
                         __func__, GetName());
||||||| parent of 1904ae3cac0 (indexes, refactor: Remove CBlockIndex* uses in coinstatsindex LookUpOne function)
        if (!LookUpOne(*m_db, pindex, entry)) {
            return false;
=======
        if (!LookUpOne(*m_db, pindex->GetBlockHash(), pindex->nHeight, entry)) {
||||||| parent of 4796a92fa9a (indexes, refactor: Remove CBlockIndex* uses in index Init methods)
        if (!LookUpOne(*m_db, pindex->GetBlockHash(), pindex->nHeight, entry)) {
=======
        if (!LookUpOne(*m_db, block->hash, block->height, entry)) {
>>>>>>> 4796a92fa9a (indexes, refactor: Remove CBlockIndex* uses in index Init methods)
            return false;
>>>>>>> 1904ae3cac0 (indexes, refactor: Remove CBlockIndex* uses in coinstatsindex LookUpOne function)
        }
