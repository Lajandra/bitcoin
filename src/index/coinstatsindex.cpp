...
||||||| parent of 2f823a9801b (indexes, refactor: Remove CBlockIndex* uses in index Init methods)
        if (!LookUpOne(*m_db, {pindex->GetBlockHash(), pindex->nHeight}, entry)) {
=======
        if (!LookUpOne(*m_db, *block, entry)) {
>>>>>>> 2f823a9801b (indexes, refactor: Remove CBlockIndex* uses in index Init methods)
            return error("%s: Cannot read current %s state; index may be corrupted",
                         __func__, GetName());
        }
