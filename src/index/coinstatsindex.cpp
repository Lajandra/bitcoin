...
||||||| parent of deecdd57b3e (indexes, refactor: Remove remaining CBlockIndex* uses in index CustomAppend methods)
using node::TxOutSer;
using node::UndoReadFromDisk;
=======
using node::TxOutSer;
>>>>>>> deecdd57b3e (indexes, refactor: Remove remaining CBlockIndex* uses in index CustomAppend methods)
...
<<<<<<< HEAD
    if (!LookUpOne(*m_db, block_index, entry)) {
        return std::nullopt;
||||||| parent of ec39653e593 (indexes, refactor: Remove CBlockIndex* uses in coinstatsindex LookUpOne function)
    if (!LookUpOne(*m_db, block_index, entry)) {
        return false;
=======
