...
||||||| parent of c23961a44e1 (indexes, refactor: Remove CBlockIndex* uses in index Rewind methods)
...
>>>>>>> c23961a44e1 (indexes, refactor: Remove CBlockIndex* uses in index Rewind methods)
...
||||||| parent of 736ee5c55bf (indexes, refactor: Remove remaining CBlockIndex* uses in index Rewind methods)
    {
        LOCK(cs_main);
        CBlockIndex* iter_tip{m_chainstate->m_blockman.LookupBlockIndex(current_tip.hash)};
        CBlockIndex* new_tip_index{m_chainstate->m_blockman.LookupBlockIndex(new_tip.hash)};
...
=======
    ReverseBlock(block);
>>>>>>> 736ee5c55bf (indexes, refactor: Remove remaining CBlockIndex* uses in index Rewind methods)
