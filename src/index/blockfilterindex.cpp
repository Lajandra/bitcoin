...
>>>>>>> 848bdd1b330 (indexes, refactor: Remove CBlockIndex* uses in index WriteBlock methods)
||||||| parent of c83928db8b1 (indexes, refactor: Remove remaining CBlockIndex* uses in index CustomAppend methods)
    BlockFilter filter(m_filter_type, *block.data, block_undo);
=======
    BlockFilter filter(m_filter_type, *block.data, block.undo_data ? *block.undo_data : CBlockUndo());
>>>>>>> c83928db8b1 (indexes, refactor: Remove remaining CBlockIndex* uses in index CustomAppend methods)
