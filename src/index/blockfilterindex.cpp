...
bool BlockFilterIndex::CustomInit(const std::optional<uint256>& block, std::optional<int> height)
{
...
bool InitBlockFilterIndex(node::NodeContext& node, BlockFilterType filter_type,
                          size_t n_cache_size, bool f_memory, bool f_wipe)
{
    auto result = g_filter_indexes.emplace(std::piecewise_construct,
                                           std::forward_as_tuple(filter_type),
                                           std::forward_as_tuple(interfaces::MakeChain(node), filter_type,
