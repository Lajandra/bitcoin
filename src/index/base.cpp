...
||||||| parent of b45a7b82660 (indexes, refactor: Move more new block logic out of ThreadSync to blockConnected)
            auto current_time{std::chrono::steady_clock::now()};
            if (last_log_time + SYNC_LOG_INTERVAL < current_time) {
                LogPrintf("Syncing %s with block chain from height %d\n",
                          GetName(), pindex->nHeight);
...
=======
>>>>>>> b45a7b82660 (indexes, refactor: Move more new block logic out of ThreadSync to blockConnected)
            CBlock block;
...
||||||| parent of 9b31c69d9a5 (indexes, refactor: Move sync thread from index to node)
static const CBlockIndex* NextSyncBlock(const CBlockIndex* pindex_prev, CChain& chain) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    AssertLockHeld(cs_main);
...
=======
>>>>>>> 9b31c69d9a5 (indexes, refactor: Move sync thread from index to node)
