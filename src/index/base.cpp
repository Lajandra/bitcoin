...
void BaseIndexNotifications::blockConnected(const interfaces::BlockInfo& block_info)
{
    ...
    if (!block.data) {
        ...
        if (block.chain_tip) {
            CallFunctionInValidationInterfaceQueue([this] { m_index.m_synced = true; });
            if (pindex) {
...
bool BaseIndex::Init()
{
    ...
    auto start_sync = [&](const interfaces::BlockInfo& block) {
        ...
        // Call CustomInit and set m_synced. It is important to call CustomInit
        // before setting m_synced to ensure that CustomInit is always called
        // before CustomAppend. CustomAppend calls from the notification thread
        // will start happening when m_synced is true.
        ...
        CallFunctionInValidationInterfaceQueue([this, synced = block.chain_tip] { m_synced = synced; });
        return true;
    };
