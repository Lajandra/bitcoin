...
||||||| parent of 506e9c47ed2 (indexes, refactor: Remove index validationinterface hooks)
    interfaces::BlockInfo block_info = kernel::MakeBlockInfo(pindex, block.get());
    if (CustomAppend(block_info)) {
        SetBestBlockIndex(pindex);
    } else {
...
=======
    return false;
>>>>>>> 506e9c47ed2 (indexes, refactor: Remove index validationinterface hooks)
}
...
||||||| parent of 81e5f239888 (indexes: Rewrite chain sync logic, remove racy init)
bool BaseIndex::IgnoreBlockConnected(const interfaces::BlockInfo& block)
{
    // During initial sync, ignore validation interface notifications, only
    // process notifications from sync thread.
...
=======
>>>>>>> 81e5f239888 (indexes: Rewrite chain sync logic, remove racy init)
...
<<<<<<< HEAD
    if (AllowPrune() && block) {
||||||| parent of fec2e4ad436 (indexes, refactor: Remove remaining CBlockIndex* pointers from indexing code)
    m_best_block_index = block;
    if (AllowPrune() && block) {
=======
    WITH_LOCK(m_mutex, m_best_block = block);
    if (AllowPrune()) {
>>>>>>> fec2e4ad436 (indexes, refactor: Remove remaining CBlockIndex* pointers from indexing code)
