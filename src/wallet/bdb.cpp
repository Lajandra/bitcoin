...
DatabaseCursor::Status BerkeleyCursor::Next(DataStream& ssKey, DataStream& ssValue)
{
    ...
    Span<const std::byte> raw_key = {reinterpret_cast<const std::byte*>(datKey.get_data()), datKey.get_size()};
    ...
    ssKey.write(raw_key);
    ssValue.clear();
    ssValue.write({reinterpret_cast<const std::byte*>(datValue.get_data()), datValue.get_size()});
    return Status::MORE;
}
...
bool BerkeleyBatch::ReadKey(DataStream&& key, DataStream& value)
{
    ...
    if (ret == 0 && datValue.get_data() != nullptr) {
        value.clear();
        value.write({reinterpret_cast<const std::byte*>(datValue.get_data()), datValue.get_size()});
        return true;
    }
    return false;
}
