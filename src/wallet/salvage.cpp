...
util::Result<void> RecoverDatabaseFile(const ArgsManager& args, const fs::path& file_path)
{
    ...
    auto database = result << MakeDatabase(file_path, options);
    ...
    if (!(result << env->Open())) return {util::Error{}, std::move(result)};
    ...
    if (keyHex != DATA_END) {
        return {util::Error{}, std::move(result), util::Warning{Untranslated("Salvage: WARNING: Unexpected end of file while reading salvage output.")}};
    } else if (ret != 0) {
        result = {util::Error{}};
    }
