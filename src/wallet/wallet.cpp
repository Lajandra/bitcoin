...
util::Result<std::shared_ptr<CWallet>, DatabaseError> LoadWalletInternal(WalletContext& context, const std::string& name, std::optional<bool> load_on_start, const DatabaseOptions& options)
{
    ...
    try {
        ...
        auto wallet = result << CWallet::Create(context, name, std::move(*database), options.create_flags);
        if (!wallet) {
        ...
        // Write the wallet setting
        result << UpdateWalletSetting(*context.chain, name, load_on_start);
...
util::Result<std::shared_ptr<CWallet>, DatabaseError> CreateWallet(WalletContext& context, const std::string& name, std::optional<bool> load_on_start, DatabaseOptions& options)
{
    ...
    // Make the wallet
    context.chain->initMessage(_("Loading wallet…").translated);
    auto create = result << CWallet::Create(context, name, std::move(*database), wallet_creation_flags);
    if (!create) {
...
util::Result<std::shared_ptr<CWallet>, DatabaseError> RestoreWallet(WalletContext& context, const fs::path& backup_file, const std::string& wallet_name, std::optional<bool> load_on_start)
{
    ...
    auto wallet = LoadWallet(context, wallet_name, load_on_start, options);
...
util::Result<std::shared_ptr<CWallet>> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags)
{
        ...
        if ((wallet_creation_flags & WALLET_FLAG_EXTERNAL_SIGNER) || !(wallet_creation_flags & (WALLET_FLAG_DISABLE_PRIVATE_KEYS | WALLET_FLAG_BLANK_WALLET))) {
            ...
            if (walletInstance->IsWalletFlagSet(WALLET_FLAG_DESCRIPTORS)) {
                ...
                for (auto spk_man : walletInstance->GetActiveScriptPubKeyMans()) {
                    if (!spk_man->SetupGeneration()) {
                        result = {util::Error{_("Unable to generate initial keys")}};
                    }
                }
            }
        }
    ...
    if (args.IsArgSet("-fallbackfee")) {
        ...
        if (!fallback_fee) {
            result.AddError(strprintf(_("Invalid amount for -fallbackfee=<amount>: '%s'"), args.GetArg("-fallbackfee", "")));
    ...
    if (args.IsArgSet("-discardfee")) {
        ...
        if (!discard_fee) {
            result.AddError(strprintf(_("Invalid amount for -discardfee=<amount>: '%s'"), args.GetArg("-discardfee", "")));
    ...
    if (args.IsArgSet("-paytxfee")) {
        ...
        if (chain && walletInstance->m_pay_tx_fee < chain->relayMinFee()) {
            return {util::Error{strprintf(_("Invalid amount for -paytxfee=<amount>: '%s' (must be at least %s)"),
                args.GetArg("-paytxfee", ""), chain->relayMinFee().ToString())}, std::move(result)};
        }
    }
    ...
    if (args.IsArgSet("-maxtxfee")) {
        ...
        if (!max_fee) {
            ...
            result.AddWarning(_("-maxtxfee is set very high! Fees this large could be paid on a single transaction."));
        }

        if (chain && CFeeRate{max_fee.value(), 1000} < chain->relayMinFee()) {
            return {util::Error{strprintf(_("Invalid amount for -maxtxfee=<amount>: '%s' (must be at least the minrelay fee of %s to prevent stuck transactions)"),
                args.GetArg("-maxtxfee", ""), chain->relayMinFee().ToString())}, std::move(result)};
        }
    ...
    if (chain && !(result << AttachChain(walletInstance, *chain, rescan_required))) {
    ...
    if (tip_height && *tip_height != rescan_height)
    {
        ...
        if (chain.havePruned() || chain.hasAssumedValidChain()) {
            ...
            if (rescan_height != block_height) {
                ...
                return {util::Error{chain.hasAssumedValidChain() ?
                     _(
...
util::Result<MigrationData> CWallet::GetDescriptorsForLegacy() const
{
    ...
    if (res == std::nullopt) {
        return {util::Error{_("Error: Unable to produce descriptors for this legacy wallet. Make sure the wallet is unlocked first")}};
    }
    return std::move(*res);
}
...
util::Result<void> DoMigration(CWallet& wallet, WalletContext& context, MigrationResult& res) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    auto data = result << wallet.GetDescriptorsForLegacy();
    ...
    if (data->watch_descs.size() > 0 || data->solvable_descs.size() > 0) {
        ...
        if (data->watch_descs.size() > 0) {
            ...
            auto watchonly_wallet = result << CreateWallet(context, wallet_name, std::nullopt, options);
            if (!watchonly_wallet) {
            ...
            result << UpdateWalletSetting(*context.chain, wallet_name, /*load_on_startup=*/true);
        }
        if (data->solvable_descs.size() > 0) {
            wallet.WalletLogPrintf("Making a new watchonly wallet containing the unwatched solvable scripts\n");

            std::string wallet_name = wallet.GetName() + "_solvables";
            auto solvable_wallet = result << CreateWallet(context, wallet_name, std::nullopt, options);
            if (!solvable_wallet) {
                return {util::Error{_("Error: Failed to create new watchonly wallet")}, std::move(result)};
            }
    ...
    if (!(result << wallet.ApplyMigrationData(*data))) {
        return {util::Error{}, std::move(result)};
    }
    return result;
}
...
util::Result<MigrationResult> MigrateLegacyToDescriptor(std::shared_ptr<CWallet>&& wallet, WalletContext& context)
{
    ...
    if (!(res << RemoveWallet(context, wallet, /*load_on_start=*/std::nullopt))) {
    ...
    DatabaseOptions options;
    options.require_existing = true;
    auto database = res << MakeWalletDatabase(wallet_name, options);
    if (!database) {
        return {util::Error{Untranslated("Wallet file verification failed.")}, std::move(res)};
    }

    auto local_wallet = res << CWallet::Create(empty_context, wallet_name, std::move(*database), options.create_flags);
    if (!local_wallet) {
    ...
    bool success = false;
    {
        ...
        if (!(res << (*local_wallet)->MigrateToSQLite())) return {util::Error{}, std::move(res)};

        // Do the migration, and cleanup if it fails
        success = bool{res << DoMigration(**local_wallet, context, *res)};
    }
    ...
    if (success) {
        ...
        local_wallet->reset();
        res << LoadWallet(context, wallet_name, /*load_on_start=*/std::nullopt, options);
        ...
        // Unload the wallets
        for (std::shared_ptr<CWallet>& w : created_wallets) {
            if (!(res << RemoveWallet(context, w, /*load_on_start=*/false))) {
        ...
        // Restore the backup
        if (!(res << RestoreWallet(context, temp_backup_location, wallet_name, /*load_on_start=*/std::nullopt))) {
