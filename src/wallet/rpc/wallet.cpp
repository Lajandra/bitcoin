        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    auto wallet = result << CreateWallet(context, request.params[0].get_str(), load_on_start, options);
    if (!wallet) {
        RPCErrorCode code = wallet.GetFailure() == DatabaseError::FAILED_ENCRYPT ? RPC_WALLET_ENCRYPTION_FAILED : RPC_WALLET_ERROR;
        throw JSONRPCError(code, util::ErrorString(result.GetErrors()).original);
    }
