...
BOOST_AUTO_TEST_CASE(rpc_namedonlyparams)
{
    ...
    // Make sure options and named parameters combine.
    BOOST_CHECK_EQUAL(TransformParams(JSON(R"({"arg1": 1, "arg2": 2, "opt2": 20, "options":{"opt1": 10}})"), arg_names).write(), R"([1,2,{"opt2":20,"opt1":10}])");
    ...
    BOOST_CHECK_EXCEPTION(TransformParams(JSON(R"({"arg1": 1, "arg2": 2, "opt1": 10, "options":{"opt1": 10}})"), arg_names), UniValue,
                          HasJSON(R"({"code":-8,"message":"Parameter opt1 specified multiple times"})"));
}
