...
||||||| parent of 248a37ac830 (refactor: Reduce number of LoadChainstate parameters)
using node::LoadChainstate;
using node::RegenerateCommitments;
using node::VerifyLoadedChainstate;
using node::fPruneMode;
...
using node::RegenerateCommitments;
using node::VerifyLoadedChainstate;
>>>>>>> 248a37ac830 (refactor: Reduce number of LoadChainstate parameters)
    ...
    : ChainTestingSetup(chainName, extra_args)
{
    ...
    auto [status, error] = LoadChainstate(*Assert(m_node.chainman.get()), m_cache_sizes, options);
